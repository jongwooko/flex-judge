...
def tokenize_messages_llama3(messages, tokenizer, think=False):
    tokenized = {
        "input_ids": [],
        "attention_mask": [],
        "labels": []
...
def tokenized_messages(messages, tokenizer, llama_type, think=False):
    if llama_type == 'llama2':
        return tokenize_messages_llama2(messages, tokenizer)
    elif llama_type == 'llama3':
        return tokenize_messages_llama3(messages, tokenizer, think=think)
    else:
        raise ValueError("Unsupported Llama type. Choose 'llama2' or 'llama3'.")

def batch_tokenize_messages_list(messages_list, tokenizer, llama_type, padding_side='left', think=False):
    tokenized_list = []
    for messages in messages_list:
        tokenized = tokenized_messages(messages, tokenizer, llama_type, think=think)
