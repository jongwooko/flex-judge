...
def main(args):
    ...
    dataset = load_dataset('json', data_files='data/pandalm/testset-v1.json', split='train')
    ...
    for example in dataset:
        ...
        input = example["input"]
        response1, response2= example["response1"], example["response2"]
    ...
    os.makedirs("results/pandalm", exist_ok=True)
    new_ckpt = args.ckpt.split('/')[-1] if '/' in args.ckpt else args.ckpt
    with open(f"results/pandalm/{new_ckpt}_{args.k}.jsonl", "w") as ans_file:
        for o in outputs:
            try:
